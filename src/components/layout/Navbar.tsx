    ...
    <>
      <header
        className={cn(
          ...
          mobileMenuOpen
            ? 'bg-brand-cream border-b border-brand-brown/10 py-3 shadow-xs'
            : isScrolled
      ...
      >
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between">
            ...
            {/* Mobile Hamburger & Quick Order Buttons */}
            <div className="flex items-center gap-2 lg:hidden">
              <Button
                href="/order-online"
                variant="primary"
                size="sm"
                className="sm:hidden px-3 py-1.5 text-xs font-bold"
                onClick={() => setMobileMenuOpen(false)}
              >
                Order
              </Button>
              ...
              >
                {mobileMenuOpen ? <X className="w-6 h-6 text-brand-orange" /> : <MenuIcon className="w-6 h-6" />}
              </button>
            </div>
          </div>
        </div>
      ...
      {/* Fullscreen Mobile Navigation Drawer
          RENDERED OUTSIDE <header> to avoid being trapped by backdrop-filter containing block when scrolled */}
      <div
        className={cn(
          'fixed inset-0 z-50 lg:hidden overflow-y-auto transition-all duration-300 ease-in-out bg-brand-cream/98 backdrop-blur-md flex flex-col justify-between p-6 pt-20 sm:pt-24 pb-8',
          mobileMenuOpen
      ...
      >
        ...
        <div className="pt-6 border-t border-brand-brown/10 space-y-4 mt-6">
          ...
          <div className="bg-white/90 p-4 rounded-2xl border border-brand-brown/10 shadow-xs">
            ...
            <div className="space-y-2 text-sm">
              {locationsConfig.map((loc) => (
                <a
                  ...
                  className="flex items-center justify-between text-brand-brown hover:text-brand-orange py-1 transition-colors"
                >
                  <span className="font-medium">{loc.name.replace(' Outlet', '')}</span>
                  <span className="text-brand-orange font-semibold flex items-center gap-1 text-xs">
